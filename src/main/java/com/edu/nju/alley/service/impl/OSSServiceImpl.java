...
import com.edu.nju.alley.vo.OSSParamVO;
...
public class OSSServiceImpl implements OSSService {
    ...
    public OSSPolicyVO policy() {
        ...
        return new OSSPolicyVO(
                ...
                dir,
                action,
                BinaryUtil.toBase64String(JSONUtil.parse(new OSSParamVO(oss.getCallback())).toString().getBytes(StandardCharsets.UTF_8)));
    }

    @Override
    public OSSResultVO callback(String filename, String size, String mimeType, String width, String height) {
        String file = "https://".concat(oss.getBucketName()).concat(".").concat(oss.getEndPoint()).concat("/").concat(filename);
        return new OSSResultVO(file, size, mimeType, width, height);
    }
